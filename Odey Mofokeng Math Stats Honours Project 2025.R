...
library(pROC)
library(glmnet)
library(doParallel)  # For parallel processing in multi-date evaluation
...
calculate_metrics <- function(predicted, actual, model_name = "Model") {
  valid_idx <- which(!is.na(predicted) & !is.na(actual))
  if (length(valid_idx) < 2) {
    warning(sprintf("Insufficient valid data for %s metrics calculation", model_name))
  ...
  mae <- mean(abs(act - pred), na.rm = TRUE)
  rmse <- sqrt(mean((act - pred)^2, na.rm = TRUE))
  smape <- mean(2 * abs(act - pred) / (abs(act) + abs(pred) + 1e-10), na.rm = TRUE) * 100
  mape <- mean(abs((act - pred) / pmax(act, 1e-10)), na.rm = TRUE) * 100
  ss_tot <- sum((act - mean(act, na.rm = TRUE))^2, na.rm = TRUE)
  ss_res <- sum((act - pred)^2, na.rm = TRUE)
  r2 <- if (ss_tot > 0) 1 - ss_res / ss_tot else NA
  
  return(list(MAE = mae, RMSE = rmse, SMAPE = smape, MAPE = mape, R2 = r2))
}
...
fit_rk_model <- function(data_sp, data_cases, data_wwtp, data_access, 
  ...
  # 17) Multi-date evaluation for RK-SEM and baselines
  multi_date_evaluation <- function(data_sp, data_cases, data_wwtp, data_access,
                                    wwtp_allocated_sp, hospital_data, eval_dates,
                                    window_days = 60, n_folds = 3, use_spatial_block = TRUE,
                                    spatial_block_km = 25, outbreak_detection = TRUE,
                                    transformation = "boxcox", model_type = "lm",
                                    zero_inflation = TRUE, verbose = FALSE,
                                    train_window = 120, test_window = 60, horizon = 30,
    ...
    if (is.null(eval_dates)) {
      eval_dates <- sort(unique(data_cases$date))
      eval_dates <- eval_dates[eval_dates >= min(data_cases$date, na.rm = TRUE) + train_window]
    }
    
    if (length(eval_dates) < 1) {
      warning("No valid evaluation dates provided")
      return(list())
    }
    
    # Export objects and load packages on each cluster
    clusterExport(cl, c("data_sp", "data_cases", "data_wwtp", "data_access",
                        "wwtp_allocated_sp", "hospital_data", "window_days", "n_folds",
                        "use_spatial_block", "spatial_block_km", "outbreak_detection",
                        "transformation", "model_type", "zero_inflation", "invBoxCox_safe",
                        "backtransform", "calculate_metrics"), envir = environment())
    ...
    results <- foreach(target_date = eval_dates, .packages = c("dplyr", "sf", "gstat", "sp", "spdep", "caret", "spatialreg", "MASS"),
                       .combine = rbind) %dopar% {
                         # Run RK-SEM for current date
                         rk_result <- tryCatch({
                           fit_rk_model(
                             data_sp = data_sp,
                             data_cases = data_cases,
                             ...
                             verbose = FALSE,
                             eval_dates = NULL  # Avoid recursive calls
                           )
                         }, error = function(e) {
                           warning(sprintf("RK-SEM failed for date %s: %s", target_date, e$message))
                           return(NULL)
                         })
                         
                         if (is.null(rk_result)) {
                           return(data.frame(Date = target_date, Model = c("RK-SEM", "Rolling Average", "LOCF", "Spatial Kriging", "Overall Average"),
                                             MAE = NA, RMSE = NA, SMAPE = NA, MAPE = NA, R2 = NA))
                         }
                         ...
                         baseline_rolling_avg <- data_cases %>%
                           filter(date >= window_start & date < as.Date(target_date)) %>%
                           group_by(SP_CODE) %>%
                           summarise(Pred_Cases = mean(Cases, na.rm = TRUE) / 1000, .groups = "drop")
                         
                         baseline_locf <- data_cases %>%
                           filter(date == max(date[date <= as.Date(target_date)], na.rm = TRUE)) %>%
                           group_by(SP_CODE) %>%
                           summarise(Pred_Cases = mean(Cases, na.rm = TRUE) / 1000, .groups = "drop")
                         
                         baseline_avg <- mean(data_cases$Cases[data_cases$date <= as.Date(target_date)], na.rm = TRUE) / 1000
                         
                         spdf <- SpatialPointsDataFrame(coords = st_coordinates(st_centroid(rk_result$model_sf)),
                                                        data = data.frame(Cases = rk_result$model_sf$Cases))
                         ...
                         rk_result$model_sf$Pred_Cases_Spatial <- replace_na(krige_pred$var1.pred, 0)
                         
                         # Join baseline predictions
                         model_sf <- rk_result$model_sf %>%
                           left_join(baseline_rolling_avg, by = "SP_CODE") %>%
                           mutate(Pred_Cases_Rolling = replace_na(Pred_Cases, 0)) %>%
                           select(-Pred_Cases) %>%
                           mutate(Pred_Cases_LOCF = baseline_locf$Pred_Cases[match(SP_CODE, baseline_locf$SP_CODE)],
                                  Pred_Cases_LOCF = replace_na(Pred_Cases_LOCF, 0),
                                  Pred_Cases_Avg = baseline_avg)
                         
                         # Calculate metrics for all models
                         metrics_rk <- calculate_metrics(rk_result$cv_predictions, rk_result$cv_actuals, "RK-SEM")
                         metrics_rolling <- calculate_metrics(model_sf$Pred_Cases_Rolling, model_sf$Cases, "Rolling Average")
                         metrics_locf <- calculate_metrics(model_sf$Pred_Cases_LOCF, model_sf$Cases, "LOCF")
                         metrics_spatial <- calculate_metrics(model_sf$Pred_Cases_Spatial, model_sf$Cases, "Spatial Kriging")
                         metrics_avg <- calculate_metrics(model_sf$Pred_Cases_Avg, model_sf$Cases, "Overall Average")
                         
                         # Combine metrics into a data frame
                         data.frame(
                           Date = target_date,
                           Model = c("RK-SEM", "Rolling Average", "LOCF", "Spatial Kriging", "Overall Average"),
                           MAE = c(metrics_rk$MAE, metrics_rolling$MAE, metrics_locf$MAE, metrics_spatial$MAE, metrics_avg$MAE),
                           RMSE = c(metrics_rk$RMSE, metrics_rolling$RMSE, metrics_locf$RMSE, metrics_spatial$RMSE, metrics_avg$RMSE),
                           SMAPE = c(metrics_rk$SMAPE, metrics_rolling$SMAPE, metrics_locf$SMAPE, metrics_spatial$SMAPE, metrics_avg$SMAPE),
                           MAPE = c(metrics_rk$MAPE, metrics_rolling$MAPE, metrics_locf$MAPE, metrics_spatial$MAPE, metrics_avg$MAPE),
                           R2 = c(metrics_rk$R2, metrics_rolling$R2, metrics_locf$R2, metrics_spatial$R2, metrics_avg$R2)
                         )
                       }
    
    if (nrow(results) == 0) {
      warning("No valid results from multi-date evaluation")
      return(list())
    }
    
    if (verbose) {
      cat("Completed multi-date evaluation for", length(unique(results$Date)), "dates\n")
      print(head(results))
    }
  ...
  if (length(valid_idx) == 0) {
    ...
    metrics <- list(MAE = NA, RMSE = NA, SMAPE = NA, MAPE = NA, R2 = NA)
  } else {
    metrics <- calculate_metrics(cv_preds_original, model_sf$Cases, "RK-SEM")
  }
  
  # 19) Outbreak detection metrics
  outbreak_metrics <- NULL
  conf_matrix <- NULL
  ...
  model_sf <- model_sf %>% left_join(baseline_rolling_avg, 
                                     by = "SP_CODE") %>% mutate(Pred_Cases_Rolling = replace_na(Pred_Cases, 0)) %>%
    select(-Pred_Cases)
  baseline_rolling_metrics <- calculate_metrics(model_sf$Pred_Cases_Rolling, model_sf$Cases, "Rolling Average")
  ...
  model_sf <- model_sf %>%
    mutate(Pred_Cases_LOCF = baseline_locf$Pred_Cases[match(SP_CODE, baseline_locf$SP_CODE)],
           Pred_Cases_LOCF = replace_na(Pred_Cases_LOCF, 0))
  baseline_locf_metrics <- calculate_metrics(model_sf$Pred_Cases_LOCF, model_sf$Cases, "LOCF")
  
  baseline_avg <- mean(data_cases$Cases[data_cases$date <= as.Date(target_date)], na.rm = TRUE) / 1000
  model_sf$Pred_Cases_Avg <- baseline_avg
  baseline_avg_metrics <- calculate_metrics(model_sf$Pred_Cases_Avg, model_sf$Cases, "Overall Average")
  ...
  if (requireNamespace("gstat", quietly = TRUE) && requireNamespace("sp", 
    ...
    baseline_spatial_metrics <- calculate_metrics(model_sf$Pred_Cases_Spatial, model_sf$Cases, "Spatial Kriging")
  } else {
    baseline_spatial_metrics <- list(MAE = NA, RMSE = NA, SMAPE = NA, MAPE = NA, R2 = NA)
  }
  
  # 21) Enhanced plotting
  plots <- list()
  tryCatch({
  ...
  # 22) Specialised COVID-Specific Validation
  epidemiological_metrics <- NULL
  if (outbreak_detection) {
    outbreak_threshold <- quantile(model_sf$Cases, 0.9, na.rm = TRUE)
    confusion_matrix <- table(Actual = model_sf$Cases[valid_idx] >= outbreak_threshold, 
                              Predicted = cv_preds_original[valid_idx] >= outbreak_threshold)
    peak_timing_diff <- which.max(model_sf$Cases) - which.max(cv_preds_original)
    actual_doubling <- if (length(model_sf$Cases) > 1)
      log(2) / coef(lm(log(pmax(model_sf$Cases + 1, 1e-10)) ~ seq_along(model_sf$Cases)))[2] else NA
    pred_doubling <- if (length(cv_preds_original) > 1) log(2) / 
      coef(lm(log(pmax(cv_preds_original + 1, 1e-10)) ~ seq_along(cv_preds_original)))[2] 
    else NA
    epidemiological_metrics <- list(
  ...
  # 23) Run multi-date evaluation if eval_dates provided
  multi_date_results <- if (!is.null(eval_dates)) {
    multi_date_evaluation(
      data_sp = data_sp,
      data_cases = data_cases,
      data_wwtp = data_wwtp,
      data_access = data_access,
    ...
    data.frame(Date = target_date,
               Model = c("RK-SEM", "Rolling Average", "LOCF", "Spatial Kriging", "Overall Average"),
               MAE = c(metrics$MAE, baseline_rolling_metrics$MAE, baseline_locf_metrics$MAE,
                       baseline_spatial_metrics$MAE, baseline_avg_metrics$MAE),
               RMSE = c(metrics$RMSE, baseline_rolling_metrics$RMSE, baseline_locf_metrics$RMSE,
                        baseline_spatial_metrics$RMSE, baseline_avg_metrics$RMSE),
               SMAPE = c(metrics$SMAPE, baseline_rolling_metrics$SMAPE, baseline_locf_metrics$SMAPE,
                         baseline_spatial_metrics$SMAPE, baseline_avg_metrics$SMAPE),
               MAPE = c(metrics$MAPE, baseline_rolling_metrics$MAPE, baseline_locf_metrics$MAPE,
                        baseline_spatial_metrics$MAPE, baseline_avg_metrics$MAPE),
               R2 = c(metrics$R2, baseline_rolling_metrics$R2, baseline_locf_metrics$R2,
                      baseline_spatial_metrics$R2, baseline_avg_metrics$R2))
  }
  
  # 24) Return comprehensive results
  results <- list(
    model_sf = model_sf,
    ...
    metrics = list(
      ...
      baselines = list(
        rolling_avg = baseline_rolling_metrics,
        locf = baseline_locf_metrics,
        spatial_kriging = baseline_spatial_metrics,
        overall_avg = baseline_avg_metrics
      )
    ),
    plots = plots,
    multi_date_results = multi_date_results,
    epidemiological_metrics = epidemiological_metrics
  )
  if (outbreak_detection) results$confusion_matrix <- conf_matrix
  class(results) <- "rk_model_result"
  
  cat("Model Metrics (Target Date:", target_date, "):\n")
  ...
  if (nrow(multi_date_results) > 0) {
    cat("Multi-Date Metrics:\n")
    print(multi_date_results)
  }
